...
const SubMenu = ({category, products}) =>{
    ...
    return(
        <div >
            <h1 onMouseEnter={()=>setIsHover(true)} onMouseLeave={()=>setIsHover(false)}>{category}</h1>            
            {isHover && 
            <div onMouseEnter={()=>setIsHover(true)} onMouseLeave={()=>setIsHover(false)} className={isHover ? styles.subMenuOn : styles.subMenuOff}>
                <div>
                    {productSubCategory.map(subcat => <div>
                       <Link to={category + `/` + subcat} key={subcat} state={{subcat, filteredProducts}}><h1>{subcat}</h1> </Link> 
                       <img src="" alt="" />
                    </div>)}
                </div>
            </div>}
